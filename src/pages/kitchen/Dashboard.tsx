...
export default function OrderSectionDashboard() {
  ...
  useEffect(() => {
    const handleGlobalKeys = (e: KeyboardEvent) => {
      ...
      // Category selection (1-5) when not focusing input
      if (document.activeElement?.tagName !== 'INPUT' && e.key >= '1' && e.key <= '5' && !e.altKey && !e.ctrlKey) {
  ...
  }, [selectedPosTable, posCart, posLoading, tables]);
      ...
      <main className="flex-1 p-6 overflow-hidden">
        <div className="h-full flex flex-col gap-6">
                  ...
                  return (
                    <div key={t.id} className="relative group flex flex-col items-center gap-1">
                      <motion.button onClick={() => setSelectedPosTable(t)} whileHover={{ scale: 1.05 }} whileTap={{ scale: 0.95 }}
                        className={`w-14 h-14 rounded-2xl border-2 flex flex-col items-center justify-center transition-all ${statusColor}`}>
          ...
          <div className="flex-1 flex gap-6 min-h-0">
            <div className="flex-1 flex flex-col gap-6 overflow-hidden">
              <div className="flex items-center gap-4">
                <div className="flex-1 flex gap-2 overflow-x-auto no-scrollbar">
                  {['All', 'Veg', 'Non-Veg', 'Starters', 'Chinese', 'Drinks'].map(cat => (
                    <button key={cat} onClick={() => {
                      if (cat === 'Veg' || cat === 'Non-Veg') setMenuVegFilter(cat === 'Veg' ? 'veg' : 'nonveg');
                      else { setMenuVegFilter('all'); setPosSearch(cat === 'All' ? '' : cat); }
                    }}
                    className={`px-6 py-3 rounded-2xl text-[10px] font-black uppercase border transition-all whitespace-nowrap ${
                ...
                <div className="relative w-64 group">
                  ...
                  <input ref={posSearchRef} type="text" placeholder="Search dish..." value={posSearch} onChange={e => setPosSearch(e.target.value)} className="w-full bg-white/5 border border-white/10 rounded-2xl pl-10 pr-4 py-3 text-xs outline-none focus:border-orange-500" />
                </div>
              </div>

              <div className="flex-1 overflow-y-auto grid grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-3 pr-2 no-scrollbar">
                {menu
                  ...
                  .map(item => (
                    <motion.button key={item.id} onClick={() => addToPosCart(item)} whileHover={{ scale: 1.02 }} whileTap={{ scale: 0.98 }}
                      className="p-4 rounded-[28px] bg-white/5 border border-white/5 hover:border-orange-500/20 text-left flex flex-col justify-between aspect-square group transition-all">
                      <div className="flex justify-between items-start">
                        <div className={`w-2 h-2 rounded-full ${item.is_veg ? 'bg-green-500' : 'bg-red-500'}`} />
                        <span className="text-[10px] font-black text-orange-500">₹{item.price}</span>
                      </div>
                      <div>
                        <h4 className="font-bold text-xs leading-tight mb-1 line-clamp-2">{item.name}</h4>
                        <p className="text-[9px] font-black uppercase text-white/20">{item.category}</p>
                      </div>
                    </motion.button>
                  ))
                }
            ...
            {/* CART */}
            <div className="w-80 bg-white/5 rounded-[40px] border border-white/10 flex flex-col overflow-hidden">
              <div className="p-6 flex items-center justify-between">
                <h3 className="text-lg font-black">Order Panel</h3>
                <button onClick={() => setPosCart([])} className="text-[10px] font-black uppercase text-white/20 hover:text-red-500">Clear</button>
              </div>
              ...
              <div className="p-6 bg-white/[0.02] border-t border-white/10 space-y-3">
                ...
                <div className="flex justify-between items-center"><span className="text-[10px] font-bold text-white/50">GST (5%)</span><span className="text-xs font-bold text-white">₹{(posCart.reduce((s, i) => s + i.price * i.quantity, 0) * 0.05).toFixed(2)}</span></div>
                <div className="flex justify-between items-center group">
                   <span className="text-[10px] font-bold text-white/50 flex items-center gap-1">Discount <span className="text-[8px] px-1 py-0.5 rounded bg-white/5 border border-white/5">(F3)</span></span>
                ...
                <div className="pt-3 border-t border-white/10 flex justify-between items-center">
                   ...
                   <span className="text-xl font-black text-orange-500">
                      ₹{Math.max(0, (posCart.reduce((s, i) => s + i.price * i.quantity, 0) * 1.05) - posDiscount).toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 })}
                   </span>
                </div>
                <button disabled={posCart.length === 0 || !selectedPosTable || posLoading} onClick={handlePlacePosOrder}
                  ...
                  {selectedPosTable ? `Place Order (${selectedPosTable.table_number})` : 'Select Table'}
                </button>
                <div className="flex items-center justify-between text-[8px] font-bold text-white/20 uppercase mt-2">
                   <span>Shift+Enter to Place</span>
                   <span>F4 Pay</span>
                   <span>F5 Print</span>
                </div>
              </div>
